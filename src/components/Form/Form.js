...
function Form({ title, firstInput, secondInput, thirdInput, button, text, textLink, link, span }) {
    const location = useLocation();

    return (
        <form className={`form ${location.pathname === '/profile' && 'form_place_profile'}`}>
            ...
            <div className={`form__labels-block ${location.pathname === '/profile' && 'form__labels-block_place_profile'}`}>
                ...
                {location.pathname === '/signup' && (
                    <label className="form__label">
                       ...
                       <input type="password" className="form__input form__input-error" />
                       <span className="form__span">{span}</span>
                    </label>
                )}
            </div>
